...
TEST_CASE("Frequency transition properties")
{
    SUBCASE("Random frequency transitions")
    {
        ...
        for(int test = 0; test < num_tests; ++test)
        {
            ...
            DeepnoteVoice voice;
            init_voice(voice, osc_count, nt::OscillatorFrequency(start_freq), nt::SampleRate(48000.0f),
                       nt::OscillatorFrequency(animation_speed));
            ...
            if(!reached_target)
            {
                INFO("Failed test " << test << ": " << start_freq << "Hz -> " << target_freq << "Hz (oscs:" << osc_count
                                    << ", speed:" << animation_speed << ") Did not reach target in " << max_samples
                                    << " samples");
            ...
            INFO("Test " << test << ": " << start_freq << "Hz -> " << target_freq << "Hz (oscs:" << osc_count
                         << ", speed:" << animation_speed << ") Final: " << final_freq << "Hz");
        }
    }
    ...
    SUBCASE("Monotonicity property for ascending frequencies")
    {
        ...
        for(int test = 0; test < 20; ++test)
        {
            ...
            DeepnoteVoice voice;
            init_voice(voice, 4, nt::OscillatorFrequency(start_freq), nt::SampleRate(48000.0f),
            ...
            {
                process_voice(voice, nt::AnimationMultiplier(1.0f), nt::ControlPoint1(0.0f),
                              nt::ControlPoint2(1.0f)); // Linear
...
TEST_CASE("Extreme parameter values")
{
    SUBCASE("Very low frequencies")
    {
        ...
        {
            DeepnoteVoice voice;
            init_voice(voice, 3, nt::OscillatorFrequency(freq), nt::SampleRate(48000.0f),
                       nt::OscillatorFrequency(1.0f));
    ...
    SUBCASE("Very high frequencies")
    {
        ...
        {
            DeepnoteVoice voice;
            init_voice(voice, 2, nt::OscillatorFrequency(freq), nt::SampleRate(48000.0f),
            ...
            for(int i = 0; i < 1000; ++i)
            {
                auto output = process_voice(voice, nt::AnimationMultiplier(1.0f), nt::ControlPoint1(0.2f),
    ...
    SUBCASE("Extreme frequency ratios")
    {
        ...
        {
            DeepnoteVoice voice;
            init_voice(voice, 4, nt::OscillatorFrequency(pair.start), nt::SampleRate(48000.0f),
            ...
            INFO(pair.description << ": " << pair.start << "Hz -> " << pair.target << "Hz, final: " << final_freq
                                  << "Hz");
        }
    }
}
    ...
    SUBCASE("Animation multiplier extremes")
    {
        ...
        for(float multiplier : extreme_multipliers)
        {
            ...
            init_voice(voice, 2, nt::OscillatorFrequency(300.0f), nt::SampleRate(48000.0f),
                       nt::OscillatorFrequency(4.0f)); // Faster base animation
            ...
            for(int i = 0; i < max_samples; ++i)
            {
                auto output = process_voice(voice, nt::AnimationMultiplier(multiplier), nt::ControlPoint1(0.25f),
                                            nt::ControlPoint2(0.75f));
...
TEST_CASE("Detuning edge cases")
{
    SUBCASE("Extreme detune values")
    {
        ...
        {
            DeepnoteVoice voice;
            init_voice(voice, 6, nt::OscillatorFrequency(440.0f), nt::SampleRate(48000.0f),
                       nt::OscillatorFrequency(1.0f), nt::DetuneHz(detune_hz));

            // Test that extreme detuning doesn't break the voice
            bool stable = true;
            for(int i = 0; i < 1000; ++i)
            {
                auto output = process_voice(voice, nt::AnimationMultiplier(1.0f), nt::ControlPoint1(0.5f),
                                            nt::ControlPoint2(0.5f));

                if(!std::isfinite(output.get()) || std::abs(output.get()) > 30.0f)
                { // More lenient bounds
                    stable = false;
                    break;
                }
    ...
    SUBCASE("Detuning with frequency transitions")
    {
        ...
        DeepnoteVoice voice;
        init_voice(voice, 4, nt::OscillatorFrequency(220.0f), nt::SampleRate(48000.0f), nt::OscillatorFrequency(3.0f),
...
TEST_CASE("Sample rate variations")
{
    SUBCASE("Different sample rates")
    {
        ...
        {
            DeepnoteVoice voice;
            init_voice(voice, 3, nt::OscillatorFrequency(440.0f), nt::SampleRate(sr), nt::OscillatorFrequency(2.0f));
            ...
            bool stable = true;
            for(int i = 0; i < 100; ++i)
            { // Fewer samples for very high sample rates
                auto output = process_voice(voice, nt::AnimationMultiplier(1.0f), nt::ControlPoint1(0.25f),
                                            nt::ControlPoint2(0.75f));
    ...
    SUBCASE("Sample rate and frequency relationship")
    {
        ...
        {
            DeepnoteVoice voice;
            init_voice(voice, 2, nt::OscillatorFrequency(300.0f), nt::SampleRate(test.sample_rate),
                       nt::OscillatorFrequency(2.0f));
