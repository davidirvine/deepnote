...
struct OfstreamCsvTraceFunctor
{
    ...
    explicit OfstreamCsvTraceFunctor(const std::string &filename) { open(filename); }
    ...
    template <typename T, typename... Args> void operator()(const T &first, Args &&...rest) const
    {
        (*out) << first << ", ";
        (*this)(rest...);
    }

    template <typename T> void operator()(const T &value) const { (*out) << value << std::endl; }
...
TEST_CASE("DeepnoteVoice log single cycle")
{
    SUBCASE("startFrequency < targetFrequency")
    {
        ...
        init_voice(voice, 1, nt::OscillatorFrequency(400.f), sample_rate, nt::OscillatorFrequency(1.f));
        ...
        {
            process_voice(voice, nt::AnimationMultiplier(1.f), nt::ControlPoint1(0.08f), nt::ControlPoint2(0.5f),
                          trace_functor);
        }
        CHECK(voice.is_at_target());
    }
    ...
    SUBCASE("startFrequency > targetFrequency")
    {
        ...
        init_voice(voice, 1, nt::OscillatorFrequency(20000.f), sample_rate, nt::OscillatorFrequency(1.f));
