...
TEST_CASE("Bezier curve mathematical properties")
{
    SUBCASE("Basic Bezier curve evaluation")
    {
        ...
        for(const auto &curve : test_curves)
        {
            ...
            if(curve.first >= 0.0f && curve.first <= 1.0f && curve.second >= 0.0f && curve.second <= 1.0f)
            {
...
TEST_CASE("Animation interpolation accuracy")
{
    ...
    {
        DeepnoteVoice voice;
        init_voice(voice, 1, nt::OscillatorFrequency(440.0f), nt::SampleRate(48000.0f), nt::OscillatorFrequency(4.0f));
        ...
        for(const auto &curve : curves)
        {
            ...
            INFO("Curve (" << curve.first << ", " << curve.second << ") - Samples: " << frequency_progression.size()
    ...
    SUBCASE("Animation timing precision")
    {
        ...
        std::vector<CurveTest> curve_tests = {
            {0.0f, 1.0f, "Linear", 800, 3000},     {0.25f, 0.75f, "Ease-in-out", 800, 3000},
            {0.42f, 0.0f, "Ease-in", 800, 3000},   {0.0f, 0.58f, "Ease-out", 800, 3000},
            {1.0f, 0.0f, "Fast-start", 800, 3000}, {0.0f, 1.0f, "Slow-start", 800, 3000},
        };

        for(const auto &test : curve_tests)
        {
            DeepnoteVoice voice;
            init_voice(voice, 2, nt::OscillatorFrequency(300.0f), nt::SampleRate(48000.0f),
...
TEST_CASE("Curve shape characteristics")
{
    SUBCASE("Ease-in curve characteristics")
    {
        ...
        INFO("Ease-in characteristics: t=0.1 -> " << values[10] << ", t=0.25 -> " << values[25] << ", t=0.75 -> "
                                                  << values[75] << ", t=0.9 -> " << values[90]);
    }
    ...
    SUBCASE("Ease-out curve characteristics")
    {
        ...
        INFO("Ease-out characteristics: t=0.1 -> " << values[10] << ", t=0.25 -> " << values[25] << ", t=0.75 -> "
                                                   << values[75] << ", t=0.9 -> " << values[90]);
    }
    ...
    SUBCASE("Ease-in-out curve characteristics")
    {
        ...
        INFO("Ease-in-out speeds: quarter=" << quarter_speed << ", mid=" << mid_speed
...
TEST_CASE("Animation smoothness validation")
{
    ...
    {
        DeepnoteVoice voice;
        init_voice(voice, 3, nt::OscillatorFrequency(200.0f), nt::SampleRate(48000.0f), nt::OscillatorFrequency(2.0f));
        ...
        {
            float mean_delta =
                std::accumulate(frequency_deltas.begin(), frequency_deltas.end(), 0.0f) / frequency_deltas.size();
            ...
            INFO("Frequency transition smoothness - Mean delta: " << mean_delta << ", StdDev: " << std_dev
    ...
    SUBCASE("Jerk analysis (third derivative)")
    {
        ...
        DeepnoteVoice voice;
        init_voice(voice, 2, nt::OscillatorFrequency(440.0f), nt::SampleRate(48000.0f), nt::OscillatorFrequency(3.0f));
    ...
    SUBCASE("Curve consistency across different ranges")
    {
        ...
        {
            DeepnoteVoice voice;
            init_voice(voice, 2, nt::OscillatorFrequency(range.first), nt::SampleRate(48000.0f),
                       nt::OscillatorFrequency(2.5f));
            ...
            INFO("Range [" << range.first << ", " << range.second << "] - Samples: " << normalized_progression.size()
...
TEST_CASE("Complex animation scenarios")
{
    ...
    {
        DeepnoteVoice voice;
        init_voice(voice, 4, nt::OscillatorFrequency(440.0f), nt::SampleRate(48000.0f), nt::OscillatorFrequency(1.5f));

        // Test rapidly changing curve parameters during animation
        std::vector<std::pair<float, float>> curves = {
            {0.0f, 1.0f}, {0.25f, 0.75f}, {0.5f, 0.5f}, {0.42f, 0.0f}, {0.0f, 0.58f},
        };
        ...
        for(int i = 0; i < 3000; ++i)
        {
            ...
            auto output = process_voice(voice, nt::AnimationMultiplier(1.0f), nt::ControlPoint1(curve.first),
    ...
    SUBCASE("Animation with extreme multipliers")
    {
        ...
        std::vector<MultiplierTest> tests = {
            ...
            {1.0f, "Normal", 8000}, {2.0f, "Fast", 4000}, {5.0f, "Very fast", 2000}, {10.0f, "Extreme fast", 1000},
        };
        ...
        for(const auto &test : tests)
        {
            ...
            {
                process_voice(voice, nt::AnimationMultiplier(test.multiplier), nt::ControlPoint1(0.25f),
            ...
            INFO(test.description << " multiplier (" << test.multiplier << ") completed in " << samples_taken
                                  << " samples");
        }
    }
    ...
    {
        DeepnoteVoice voice;
        init_voice(voice, 2, nt::OscillatorFrequency(220.0f), nt::SampleRate(48000.0f), nt::OscillatorFrequency(3.0f));
        ...
        for(const auto &stage : stages)
        {
            ...
            INFO("Stage to " << target_freq << "Hz with curve (" << cp1 << ", " << cp2 << ") completed in "
