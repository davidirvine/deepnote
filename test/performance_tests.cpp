    ...
    SUBCASE("Multiple voice scaling") {
        ...
        for(int voice_count : voice_counts) {
            ...
            for(int i = 0; i < voice_count; ++i) {
                ...
                init_voice(voices[i], 3, nt::OscillatorFrequency(base_freq), 
            ...
            INFO("Voice count: " << voice_count << ", Processing time: " << processing_time_ms 
                 << "ms, Audio time: " << audio_time_ms << "ms, CPU usage: " << cpu_usage_percent << "%");
...
TEST_CASE("Memory allocation stability") {
    SUBCASE("No dynamic allocation during processing") {
        DeepnoteVoice voice;
        init_voice(voice, 6, nt::OscillatorFrequency(330.0f), 
    ...
    SUBCASE("Consistent memory usage across transitions") {
        ...
        const std::vector<float> target_frequencies = {
            220.0f, 440.0f, 330.0f, 880.0f, 550.0f, 
        ...
        for(int transition = 0; transition < num_transitions; ++transition) {
            ...
            INFO("Transition " << transition << " to " << target_freq 
                 << "Hz completed in " << samples_processed << " samples");
...
TEST_CASE("Stress testing") {
    SUBCASE("Rapid parameter changes") {
        DeepnoteVoice voice;
        init_voice(voice, 8, nt::OscillatorFrequency(440.0f), 
                  nt::SampleRate(48000.0f), nt::OscillatorFrequency(5.0f)); // Fast animation
        ...
        auto final_output = process_voice(voice, nt::AnimationMultiplier(1.0f),
    ...
    SUBCASE("Extreme oscillator count") {
        ...
        DeepnoteVoice voice;
        init_voice(voice, 16, nt::OscillatorFrequency(440.0f), 
                  nt::SampleRate(48000.0f), nt::OscillatorFrequency(1.0f));
...
TEST_CASE("Performance regression detection") {
    ...
    SUBCASE("Performance consistency across states") {
        ...
        for(int i = 0; i < 100; ++i) {
            ...
            if(voice.get_state() != DeepnoteVoice::State::PENDING_TRANSIT_TO_TARGET) break;
        }
